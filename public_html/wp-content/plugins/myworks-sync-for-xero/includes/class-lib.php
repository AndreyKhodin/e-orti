...
class MyWorks_WC_Xero_Sync_Lib extends MyWorks_WC_Xero_Sync_Core {
	...
	protected function myworks_wc_xero_sync_check_license($licensekey,$localkey,$realtime){
		...
		if(!empty($lc_url)){
			...
			if(!empty($licensekey) && !empty($site_url) && strlen($licensekey) == '35' && $this->validate_connection_key($licensekey)){
				$licensing_secret_key = 'XF9CY3KSP3XA8H';
				$localkeydays = 5;
	...
	public function xero_get_tax_rates_kva(){
		if($this->is_xero_connected()){
			...
			$TaxRates = $this->X_API_I()->getTaxRates($this->XeroTenantId,$X_Whr,null,null,1);
					...
					if(is_array($dc_gt_fees) && !empty($dc_gt_fees)){
						...
						if(!empty($dxtp_code)){
							foreach($dc_gt_fees as $df){
								...
								# Line Tax
								if($Tax_Sync_Type == 'Xero_Tax'){
									$TaxType = $this->get_xero_tax_code_from_line_tax_data($sd,'fee');								
								}else{
									$TaxType = $this->get_xero_non_taxable_tax_code();
								}
	...
	public function X_Pull_Price(){
		if($this->is_xero_connected()){
			...
			$last_timestamp = $this->get_option('mw_wc_xero_last_price_pull_timestamp');
			...
			if(!empty($last_timestamp)){
				$this->update_option('mw_wc_xero_last_price_pull_timestamp',$last_timestamp);
			}
			...
			if(is_array($map_data)){
				foreach($map_data as $map_data_c){
					...
					if($UnitPrice != $_price){
						...
						$this->save_log(array('type'=>'Price','title'=>$log_title,'details'=>$log_details,'status'=>1,'wc_id'=>$wc_product_id,'xero_id'=>$ItemID),true);
					}else{
						# Same Price
					}
				}
